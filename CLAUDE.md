...
- `cargo-nocturne build` resolves the target dir via `cargo metadata`, so it works from any directory inside the workspace; `./target` relative to CWD is only the fallback outside a cargo project.
