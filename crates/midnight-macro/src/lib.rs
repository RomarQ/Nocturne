    ...
    match midnight_ir::parse_contract(module.clone()) {
        Ok(contract_ir) => {
            ...
            if let Err(e) = write_artifacts(&artifact_dir, &artifacts) {
                eprintln!("midnight-edsl: warning: failed to write artifacts to {}: {e}",
            ...
            let deploy_mod =
                midnight_codegen::deploy_codegen::generate_deploy_module(&contract_ir);

            // Return the module with midnight attributes stripped +
            // generated modules injected.
...
fn strip_midnight_attrs_from_module(mut module: syn::ItemMod) -> syn::ItemMod {
    if let Some((brace, ref mut items)) = module.content {
        let cleaned_items: Vec<syn::Item> = items
            .drain(..)
            .map(|item| strip_item_attrs(item))
            .collect();
...
fn find_artifact_dir(contract_name: &str) -> std::path::PathBuf {
    ...
    let target = std::env::var("CARGO_TARGET_DIR")
        .or_else(|_| std::env::var("OUT_DIR").map(|d| {
            // OUT_DIR is like target/debug/build/<crate>/out -- walk up to target/
            let p = std::path::PathBuf::from(d);
            p.ancestors().nth(4).unwrap_or(&p).to_path_buf().to_string_lossy().into_owned()
        }))
        .unwrap_or_else(|_| "target".to_string());
    std::path::PathBuf::from(target).join("midnight").join(contract_name)
}
...
fn write_artifacts(
    ...
    for circuit in &artifacts.zkir.circuits {
        let mut value = serde_json::to_value(&circuit.ir_source)
        ...
        let json = serde_json::to_string_pretty(&value)
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
        std::fs::write(zkir_dir.join(format!("{}.zkir", circuit.circuit_name)), json)?;
    }
