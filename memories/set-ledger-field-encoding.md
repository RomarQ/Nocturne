...
**Status**: Implemented end-to-end. Tests: `set_{insert,contains,remove}_proves_and_verifies` in `crates/nocturne/tests/ledger_integration_test.rs`.
...
- **Storage** (`crates/nocturne-storage/src/set.rs`): `Set<T: Eq + Hash + Clone>` backed by `HashSet<T>`. HashSet-style API: `contains(&T) -> bool`, `insert(T) -> bool`, `remove(&T) -> bool`.
