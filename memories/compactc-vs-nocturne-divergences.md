...
|---|---|---|---|
| `Counter` | `ledger x: Counter;` | `Counter` field on `#[midnight(ledger)]` struct | Same |
...
2. Limiting goldens to the matching-structure subset and using the midnight-ledger integration test (`crates/midnight/tests/ledger_integration_test.rs`) as the general on-chain compatibility gate.
