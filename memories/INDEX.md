...
| [merkle-tree-encoding.md](merkle-tree-encoding.md) | Investigation memory + phased plan for `MerkleTree<H, T>`: 2-element Array storage shape, leafHash domain separator, Root/Eq opcodes, Field alignment. Phases A+B landed (Cell<Field>, storage type, MerkleTreeDigest, MerkleLeaf adapter); C-E pending. |
