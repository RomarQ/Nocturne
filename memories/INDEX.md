...
| [merkle-tree-encoding.md](merkle-tree-encoding.md) | Investigation memory + phased plan for `MerkleTree<H, T>`: 2-element Array storage shape, leafHash domain separator, Root/Eq opcodes, Field alignment. Phases A+B+C landed (Cell<Field>, storage type, MerkleTreeDigest, MerkleLeaf adapter, check_root end-to-end); D+E pending (insert + MerkleTreePath). |
