...
| [map-ledger-field-encoding.md](map-ledger-field-encoding.md) | Empirical compactc emission for `Map<K, V>` insert/lookup/member, opcode reference. Stages 1 (`contains`) and 3 (`insert`) landed on-chain compatible; get/remove pending. |
| [storage-cell-encoding-gap.md](storage-cell-encoding-gap.md) | `Cell::set` on-chain encoding closed for typed primitives (bool, Uint<N≤253>, integer types) via Push+Push+Ins. Multi-Fr value types and `Field` still fall back to 2-declare placeholder. Map::insert pending. |
