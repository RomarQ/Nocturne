...
| [merkle-tree-encoding.md](merkle-tree-encoding.md) | Investigation-only memory mapping the full `MerkleTree<H, T>` on-chain encoding: 2-element Array storage shape, leafHash domain separator, Root/Eq opcodes, Field alignment. Phased plan (A-E). Phase A landed, B-E pending. |
