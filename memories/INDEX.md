...
| [scope-blockers.md](scope-blockers.md) | Feature work blocked on upstream coordination, design decisions, or explicit owner policy — env context, sum-type encoding, crate rename, cross-contract, ZKIR v3, deploy-time container encoding. |
...
| [merkle-tree-encoding.md](merkle-tree-encoding.md) | Investigation memory + phased plan for `MerkleTree<H, T>`. Phases A-E all landed (Cell<Field>, storage, MerkleTreeDigest with full-Fr representation, check_root, insert, MerkleTreePath + merkle_tree_path_root). Constructor IR for the initial Array still deferred. |
...
| [uint-arithmetic-semantics.md](uint-arithmetic-semantics.md) | In-circuit `Uint<N>` `+`/`-`/`*` is unconstrained field arithmetic (no wrap, no overflow check); off-chain `Uint` panics on overflow/underflow so test mode can't mask the divergence. Checked-constrain emission is an open decision. |
...
| [storage-cell-encoding-gap.md](storage-cell-encoding-gap.md) | All Cell ops (`set`/`get`) on-chain compatible for typed primitives, `Bytes<N>` (multi-Fr Push + Popeq), and `Field`; `Map<Bytes<N>, _>` landed too. Custom ADT cell payloads are the remaining 2-declare fallback. |
| [option-and-array-encoding.md](option-and-array-encoding.md) | `Option<T>` = `(Bytes<1>, T)` and `[T; N≤11]` = N-tuple of T via upstream tuple Aligned. Witness alloc is whole-array on first touch. Let-bound + ledger-stored arrays still open. |
