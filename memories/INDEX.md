...
| [storage-cell-encoding-gap.md](storage-cell-encoding-gap.md) | `Cell::set`/`Cell::get`/`Counter::value` on-chain encoding closed for typed primitives and `Cell<Bytes<N>>::set` via multi-Fr Push. Multi-Fr Cell::get / Map<Bytes<N>, _> and `Field` cells still pending. |
